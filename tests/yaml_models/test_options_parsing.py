...
from limbo_core.yaml_schema.tables import (
...
def test_users_yaml_options():
    ...
    assert isinstance(
        created_at.options["start_date"], ColumnOptionPrimitiveValue
    )
    assert created_at.options["start_date"].value == "2020-01-01"
    assert isinstance(
        created_at.options["end_date"], ColumnOptionPrimitiveValue
    )
    assert created_at.options["end_date"].value == "2025-01-01"

    updated_at = _get_column(table, "updated_at")
    assert isinstance(
        updated_at.options["start_date"], ColumnOptionPrimitiveValue
    )
    assert updated_at.options["start_date"].value == "2020-01-01"
    assert isinstance(
        updated_at.options["end_date"], ColumnOptionPrimitiveValue
    )
    assert updated_at.options["end_date"].value == "2025-01-01"


def test_posts_yaml_options():
    yaml_path = Path(__file__).parent / "test_project" / "models" / "posts.yaml"
    schema = _load_schema(yaml_path)
    table = schema.tables[0]

    user_id = _get_column(table, "user_id")
    assert isinstance(user_id.options["reference"], ColumnOptionReferenceValue)
    assert user_id.options["reference"].ref == "users.id"

    title = _get_column(table, "title")
    assert isinstance(title.options["prompt"], ColumnOptionPrimitiveValue)
    assert "Generate a title" in title.options["prompt"].value

    created_at = _get_column(table, "created_at")
    assert isinstance(
        created_at.options["start_date"], ColumnOptionReferenceValue
    )
    assert created_at.options["start_date"].ref == "users.created_at"
    assert isinstance(
        created_at.options["end_date"], ColumnOptionPrimitiveValue
    )
    assert created_at.options["end_date"].value == "2025-01-01"
    ...
    updated_at = _get_column(table, "updated_at")
    assert isinstance(
        updated_at.options["start_date"], ColumnOptionReferenceValue
    )
...
def test_comments_yaml_options():
    yaml_path = (
    ...
    assert isinstance(post_id.options["reference"], ColumnOptionReferenceValue)
    assert post_id.options["reference"].ref == "posts.id"
    ...
    assert isinstance(
        user_full_name.options["reference"], ColumnOptionReferenceValue
    )
    assert user_full_name.options["reference"].ref == "users.full_name"

    created_at = _get_column(table, "created_at")
    assert isinstance(
        created_at.options["start_date"], ColumnOptionReferenceValue
    )
    assert created_at.options["start_date"].ref == "posts.created_at"
    assert isinstance(
        created_at.options["end_date"], ColumnOptionPrimitiveValue
    )
    assert created_at.options["end_date"].value == "2025-01-01"
